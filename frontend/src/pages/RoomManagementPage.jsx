...
function RoomManagementPage() {
  ...
  const roomOverview = useMemo(() => {
    ...
    const availableRooms = rooms.reduce((sum, room) => {
      const totalForRoom = Math.max(1, Number(room.totalRooms || 1));
      const normalizedRemaining = Number.isFinite(Number(room.remainingRooms))
        ? Number(room.remainingRooms)
        : totalForRoom;
      const availableForRoom = Math.min(
        totalForRoom,
        Math.max(0, normalizedRemaining),
      );
    ...
    bookings.forEach((booking) => {
      ...
      bookedByRoomNumber.set(
        roomKey,
        (bookedByRoomNumber.get(roomKey) || 0) + bookedCount,
      );
    });

    const bookedRooms = Array.from(bookedByRoomNumber.values()).reduce(
      (sum, count) => sum + count,
      0,
    );
  ...
  const loadLatestRecords = async () => {
    const [roomRes, bookingRes] = await Promise.allSettled([
      getRooms(),
      getRoomBookings(),
    ]);
    ...
    if (bookingRes.status === "fulfilled") {
      ...
      setBookingError(
        apiMessage || "Unable to refresh booking records right now.",
      );
    }
  };
  ...
  const handleRoomSubmit = async (e) => {
    ...
    try {
      const requiredChecks = [
        ...
        {
          label: "Total Rooms",
          value: String(roomForm.totalRooms ?? "").trim(),
        },
        {
          label: "Normal Price",
          value: String(roomForm.normalPrice ?? "").trim(),
        },
        {
          label: "Weekend Price",
          value: String(roomForm.weekendPrice ?? "").trim(),
        },
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      ...
      if (editingBookingId) {
        ...
        const res = await createRoomBooking(payload);
        ...
        try {
          ...
          console.log("Failed to trigger booking housekeeping task:", error);
        }
      }
  ...
  return (
    <>
          ...
          <div className="form-grid">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setRoomForm({ ...roomForm, photoUrl: e.target.value })
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setRoomForm({ ...roomForm, normalPrice: e.target.value })
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setRoomForm({ ...roomForm, weekendPrice: e.target.value })
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setRoomForm({ ...roomForm, seasonalPrice: e.target.value })
                }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setRoomForm({ ...roomForm, roomStatus: e.target.value })
                }
                required
              >
            ...
            <div className="span-full">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setRoomForm({ ...roomForm, roomDescription: e.target.value })
                }
      ...
      <div className="card" style={{ marginTop: "16px" }}>
        <h3>
          {editingBookingId ? "Edit Room Booking" : "Create Room Booking"}
        </h3>
          ...
          <div className="form-grid">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setBookingForm({
                    ...bookingForm,
                    bookingCustomer: e.target.value,
                  })
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setBookingForm({ ...bookingForm, roomNumber: e.target.value })
                }
                placeholder="204"
                required
              />
                ...
                <tbody>
                  {rooms.map((room) => (
                    <tr key={room.id}>
                      ...
                      <td>
                        Rs. {Number(room.normalPrice || 0).toLocaleString()}
                      </td>
                      <td>
                        <button
                          className="btn small"
              ...
              <table>
                ...
                <tbody>
                  {bookings.map((booking) => (
                    <tr key={booking.id}>
                      <td>
                        {String(booking.bookingSequence ?? booking.id).padStart(
                          2,
                          "0",
                        )}
                      </td>
